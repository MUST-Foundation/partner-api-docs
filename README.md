...
Надо учитывать, что если для агента есть возможность оформить полис в нескольких СК, то при начале скоринга агент получит уведомление с набором СК, и на каждую СК в отдельности получит результат скоринга. Результат скоринга по каждой отдельной СК будет приходить по мере ответа СК.  
Результат скоринга по всем СК будет определяться отправкой одного из трех хуков по каждой СК (успешно/отказ/ошибка СК)
...
Для Юридических лиц добавлена возможность выпуска полиса и оплата по счету. 
Агент может выбрать либо формирование ссылки, либо формирование счета на оплату.
Загрузку контракта следует выполнять после загрузки документов, т.к. после распознавания данные могут затереться.

## Обновлены методы / хуки

### /api/scoring/{scoringId}/contract

Для ЮЛ обязательны поля для заполнения ```inn, kpp```

### /api/scoring/{scoringId}/documents

После успешной загрузки всех документов происходит распознавание, 
система отправит хук с предзаполненным контрактом /api/hooks/{scoringId}/contract/update-completed.

### /api/scoring/{scoringId}/payment-invoice

Добавился новый метод для создания счета на оплату.

Когда счет на оплату будет сформирована, то система направит хук /api/hooks/{scoringId}/payment-invoice-generated

### /api/scoring/scoring/{scoringId}/payment-order

Добавился новый метод для загрузки платежного поручения.

После успешной проверки платежного поручения система направит хук /api/hooks/{scoringId}/policy-is-paid
